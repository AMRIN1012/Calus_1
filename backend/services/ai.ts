...
import { Transaction, AssessmentResult } from "../types.js";
...
export async function getAIAnalysis(transaction: Transaction, ruleResult: AssessmentResult): Promise<string> {
  if (!genAI) {
    ...
    if (!apiKey) {
      return "[SYSTEM] AI analysis requires a GEMINI_API_KEY in the environment. Please add it to your .env file.";
    }
  ...
  try {
    ...
    return response.response.text() || "AI Analysis unavailable at this time.";
  } catch (error) {
